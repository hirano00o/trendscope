...
企業データのCRUD操作、効率的な差分処理、バッチ更新機能を提供
"""
...
class DatabaseService:
    ...
    SQLiteデータベースを使用して企業情報を管理する。

    Attributes:
        db_connection: データベース接続管理オブジェクト

    Example:
        >>> conn = DatabaseConnection("stocks.db")
    ...
    def setup_database(self) -> None:
        ...
        Example:
            ...
            ...     service.setup_database()
        """
        migration = DatabaseMigration(self.db_connection)
    ...
    def get_company_by_symbol(self, symbol: str) -> Company | None:
        ...
        try:
            ...
            WHERE symbol = ?
            """
            cursor = self.db_connection.execute_query(sql, (symbol,))
            row = cursor.fetchone()
    ...
    def get_all_companies(self) -> list[Company]:
        ...
        try:
            ...
            ORDER BY symbol
            """
            cursor = self.db_connection.execute_query(sql)
    ...
    def get_companies_by_market(self, market: str) -> list[Company]:
        ...
        try:
            ...
            ORDER BY symbol
            """
            cursor = self.db_connection.execute_query(sql, (market,))
            rows = cursor.fetchall()
    ...
    def get_database_stats(self) -> dict[str, Any]:
        ...
        try:
            # 総企業数
            cursor = self.db_connection.execute_query("SELECT COUNT(*) FROM company")
            ...
            # 最終更新日時
            cursor = self.db_connection.execute_query(
                "SELECT MAX(last_updated) FROM company"
            )
