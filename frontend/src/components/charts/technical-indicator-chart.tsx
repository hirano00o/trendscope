...
const TechnicalTooltip: React.FC<TechnicalTooltipProps> = ({ active, payload, label, indicatorType }) => {
    ...
    <div style={{ height }}>
        <ResponsiveContainer width="100%" height="100%">
            <LineChart data={data} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                ...
                <YAxis domain={[0, 100]} tick={{ fontSize: 11, fill: theme.text }} stroke={theme.text} />
                ...
                <Line type="monotone" dataKey="rsi" stroke={theme.primary} strokeWidth={2} dot={false} name="RSI" />
            </LineChart>
        </ResponsiveContainer>
    </div>
)
    ...
    <div style={{ height }}>
        <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={data} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                <CartesianGrid strokeDasharray="3 3" stroke={theme.grid} opacity={0.3} />
                <XAxis dataKey="date" tick={{ fontSize: 11, fill: theme.text }} stroke={theme.text} />
                <YAxis tick={{ fontSize: 11, fill: theme.text }} stroke={theme.text} />
                ...
                {/* MACD Histogram */}
                <Bar dataKey="macdHistogram" fill={theme.neutral} opacity={0.3} name="MACD Histogram" />

                {/* MACD Lines */}
                <Line type="monotone" dataKey="macd" stroke={theme.primary} strokeWidth={2} dot={false} name="MACD" />
                <Line
                    type="monotone"
                    dataKey="macdSignal"
            ...
            <div className="space-y-4">
                {activeIndicators.includes("rsi") && (
                    <div>
                        ...
                        <RSIChart data={chartData} theme={mergedTheme} height={panelHeight} />
                    </div>
                )}
                ...
                {activeIndicators.includes("macd") && (
                    <div>
                        ...
                        <MACDChart data={chartData} theme={mergedTheme} height={panelHeight} />
                    </div>
                )}

                {activeIndicators.includes("momentum") && (
                    <div>
                        <h4 className="text-sm font-medium text-neutral-700 mb-2">Momentum - Price Rate of Change</h4>
                        <div style={{ height: panelHeight }}>
                            <ResponsiveContainer width="100%" height="100%">
                                <LineChart data={chartData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                                    ...
                                    />
                                    <YAxis tick={{ fontSize: 11, fill: mergedTheme.text }} stroke={mergedTheme.text} />
