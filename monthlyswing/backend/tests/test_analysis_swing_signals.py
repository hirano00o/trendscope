...
class TestBasicSignalGeneration:
    ...
    def test_generate_wait_signal_from_unclear_trend(self):
        ...
        assert any(
            "不明確" in factor or "様子見" in factor
...
class TestPriceLevelCalculation:
    ...
    def test_calculate_target_price_from_resistance_levels(self):
        ...
        assert target_price >= current_price * (
            Decimal("1.0") + expected_return_rate * Decimal("0.8")
        )  # 期待リターンの80%以上
...
class TestConfidenceAndFactorEvaluation:
    ...
    def test_generate_supporting_factors_automatically(self):
        ...
        # 価格目標の説明が含まれているか
        price_target_explained = any(
            "ターゲット" in factor or "目標" in factor for factor in factors
        )
...
class TestIntegratedSignalGenerator:
    ...
    def test_complete_signal_generation_workflow(self):
        ...
        from monthlyswing_backend.analysis.swing_signals import generate_complete_signal

        # テストデータ準備：動作するMonthlyTrendResult形式
            ...
            support_resistance={
                ...
                "resistance": [
                    SupportResistanceLevel(
                        ...
                        confidence=Decimal("0.80"),
                        touch_count=2,
                        last_touch_date=datetime(2024, 1, 25),
                        level_type="レジスタンス",
                        strength_score=Decimal("0.75"),
                    )
                ],
        ...
        complete_signal = generate_complete_signal(
            monthly_trend_result=trend_result,
            technical_confidence=Decimal("0.85"),
        ...
        # 基本検証
        assert complete_signal is not None
        assert isinstance(complete_signal, SwingSignal)
        assert complete_signal.signal_type in [
            SignalType.BUY,
            SignalType.SELL,
            SignalType.HOLD,
            SignalType.WAIT,
        ]
        assert Decimal("0.0") <= complete_signal.confidence <= Decimal("1.0")

        # シグナル内容検証
        assert len(complete_signal.supporting_factors) >= 6  # 統合版は最低6つの根拠
        assert complete_signal.generated_at is not None

        # 強いアップトレンドなのでBUYシグナルが期待される
        assert complete_signal.signal_type == SignalType.BUY
        assert complete_signal.confidence >= Decimal("0.8")  # 高信頼度

        # 価格レベル情報が含まれているか確認
        price_info_found = any(
            "ターゲット" in factor or "ストップ" in factor
            for factor in complete_signal.supporting_factors
        )
        assert price_info_found

        # 複合信頼度情報が含まれているか確認
        confidence_info_found = any(
            "信頼度" in factor for factor in complete_signal.supporting_factors
        )
        assert confidence_info_found
    ...
    def test_signal_generator_error_handling(self):
        ...
        trend_result = MonthlyTrendResult(
            ...
            trend_strength=TrendStrengthMetrics(
                ...
                consistency=Decimal("0.7"),
                duration_days=20,
            ),
            support_resistance={"support": [], "resistance": []},
            continuation_probability=Decimal("0.7"),
        )

        with pytest.raises(ValueError):
            generate_complete_signal(
                monthly_trend_result=trend_result,
                technical_confidence=Decimal("1.5"),  # 範囲外
                pattern_confidence=Decimal("0.7"),
                volume_confidence=Decimal("0.9"),
            )

    def test_complete_signal_with_minimal_data(self):
        """最小限のデータでの統合シグナル生成.

        Given:
            - 最小限のMonthlyTrendResult
            - 基本的な信頼度パラメータ
        ...
        # 最小限のデータ
        minimal_trend_result = MonthlyTrendResult(
            symbol="MINIMAL",
            data_period={
                "start_date": datetime(2024, 1, 1),
                "end_date": datetime(2024, 1, 31),
            ...
            trend_strength=TrendStrengthMetrics(
                ...
                consistency=Decimal("0.4"),
                duration_days=10,
            ),
            support_resistance={"support": [], "resistance": []},  # 空リスト
            continuation_probability=Decimal("0.5"),
        )

        # 統合シグナル生成（最小パラメータ）
        signal = generate_complete_signal(
            monthly_trend_result=minimal_trend_result,
            technical_confidence=Decimal("0.6"),
            pattern_confidence=Decimal("0.5"),
            volume_confidence=Decimal("0.6"),
        )
        ...
        # 横這いトレンドの場合、HOLDまたはWAITが期待される
        assert signal.signal_type in [SignalType.HOLD, SignalType.WAIT]

        # 低い信頼度でも有効な範囲内
        assert Decimal("0.0") <= signal.confidence <= Decimal("1.0")

        # 最低限の根拠が生成される
        assert len(signal.supporting_factors) >= 3  # 最小限でも3つ以上
